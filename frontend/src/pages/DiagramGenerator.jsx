...
export default function DiagramGenerator() {
  ...
  // Pan handlers for interactive diagram movement
      ...
      if (svg) {
        ...
        img.onload = () => {
          ...
          const pdf = new jsPDF({
            ...
            unit: 'px',
            format: [canvas.width + 80, canvas.height + 120]
          });
          
          // Add title
          pdf.setFontSize(18);
          pdf.setTextColor(6, 182, 212); // Primary cyan color
          pdf.text(currentDiagram?.title || "Architecture Diagram", 40, 40);
          
          // Add date
          pdf.setFontSize(10);
          pdf.setTextColor(100, 116, 139);
          pdf.text(new Date().toLocaleDateString(language === 'fr' ? 'fr-FR' : 'en-US'), 40, 60);
          ...
          pdf.setFontSize(8);
          pdf.setTextColor(100, 116, 139);
          pdf.text('Generated by ArchAdvisor - © Malek Berrezouga', 40, canvas.height + 100);
          
          // Save PDF
          pdf.save(`${currentDiagram?.title || "diagram"}.pdf`);
          ...
          <Card className="card-tech h-full">
            <CardHeader className="border-b border-white/5 flex flex-row items-center justify-between py-3">
              ...
              <div className="flex items-center gap-2">
                {/* Zoom Controls */}
                <div className="flex items-center gap-1 mr-2 bg-slate-800/50 rounded-lg px-2 py-1">
                  <Button
                    variant="ghost"
                    size="icon"
                  ...
                  <span className="text-xs text-slate-400 w-12 text-center font-mono">{zoom}%</span>
                  <Button
                    variant="ghost"
                    size="icon"
                    ...
                    <DialogHeader>
                      <DialogTitle className="text-white flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-500 ml-2">
                          (Ctrl + Scroll pour zoomer, Glisser pour déplacer)
                        </span>
                      </DialogTitle>
                    </DialogHeader>
                    <div 
                      className="flex-1 overflow-hidden bg-slate-900/50 rounded-lg cursor-grab active:cursor-grabbing"
            ...
            <CardContent className="p-0 h-[calc(100%-60px)]">
              {currentDiagram ? (
                <div 
                  className="w-full h-full overflow-hidden bg-slate-900/30 cursor-grab active:cursor-grabbing relative"
                ...
                >
                  {/* Pan indicator */}
                  <div className="absolute top-2 left-2 flex items-center gap-1 text-xs text-slate-500 bg-slate-800/70 px-2 py-1 rounded-md">
                    <Move className="w-3 h-3" />
                    <span>{t.diagrams.dragToMove}</span>
                  </div>
